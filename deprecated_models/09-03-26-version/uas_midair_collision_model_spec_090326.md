...
## 13. Exposure-Rate Layer Major Revision (Cross-Reference)

The companion exposure-rate spec's K formula (which multiplies against this document's p_3a/p_3b to produce the combined tool's headline output) was substantially revised: a bug that let the exposure-rate layer's Scenario 3a/3b/3c rates grow unboundedly with loiter duration (instead of saturating once loiter exceeds the maximum possible delivery-flight duration) was found and fixed, and all exposure-rate constants were refit against the corrected simulation. This fine-grained model's own files (`mc_prototype_orbit.py`, `hover_vs_orbit.py`) and the p_3a/p_3b values they produce are **unaffected** — the bug and fix are entirely in the exposure-rate layer (`exposure_model.py`), not here. See companion spec Sections 12.5-12.6 for the full bug description, fix, and refit constants, and Section 17 for a three-way validation against an independently-built peer model (Ben Pelletier's "expanded bird-strike" model), which also surfaced a ~10× residual gap between the two models' fine-grained collision-probability methodologies — logged as the explicit next investigation for *this* document, not yet resolved.
