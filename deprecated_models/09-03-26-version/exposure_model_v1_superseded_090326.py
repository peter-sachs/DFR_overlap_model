...
def simulate_one_day(params, rng):
    ...
    for i, route in enumerate(routes):
        ...
        for ev in dfr_events:
            # cheap temporal pre-filter: does route's absolute active window
            # overlap the loiter window at all?
            route_abs_start = t0_s
            route_abs_end = t0_s + route['total_duration_s']

            # --- 3c: DFR TRANSIT phase vs this delivery route ---
            if not (route_abs_end < ev['transit_start'] or route_abs_start > ev['transit_end']):
            ...
            if route_abs_end < ev['t_start'] or route_abs_start > ev['t_end']:
                continue
